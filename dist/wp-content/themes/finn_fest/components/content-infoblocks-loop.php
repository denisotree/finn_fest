...
if ($infoblocks_query->have_posts())
{
    ...
    <div class="container">
        ...
        while ($infoblocks_query->have_posts())
        {
            ...
            <section class="content-block <?= $n % 2 ? "content-block__text-right" : "content-block__text-left" ?>" id="line">
                <div class="content-block__text">
                    <h2><?= $title ?></h2>
                    <ul class="nav tabs-menu-universal universal-menu" id="myTab" role="tablist">
                        <?php foreach($tabs as $i => $tab) {?>
                            <li>
                                <a class="<?php if ($i == 0) echo 'active'?>" data-toggle="tab" href="#<?= $tab['tab_nav_link'] ?>" role="tab" aria-controls="<?= $tab['tab_nav_link'] ?>" <?php if ($i == 0) echo 'aria-selected="true"'?>><?= $tab['tab_nav_text'] ?></a>
                            </li>
                        <?php } ?>
                    </ul>
                    <div class="tab-content universal__description">
                        <?php foreach($tabs as $i => $tab) {?>
                            <div class="tab-pane <?php if ($i == 0) echo 'active'?>" id="<?= $tab['tab_nav_link'] ?>" role="tabpanel">
                    ...
                    <?php if ($is_button) {?>
                        ...
                        <div class="share">
                            <a href="#" onclick="yaCounter46915254.reachGoal('buyTicket1DayIndividual'); return true;" class="banner__btn-action" data-tikettype="Тур на 1 день - индивидуально" data-toggle="modal" data-target="#modal-buyticket">
